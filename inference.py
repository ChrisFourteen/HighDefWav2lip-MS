...
parser.add_argument('--checkpoint_path', type=str, default='checkpoint_step000635000.pth',
					help='Name of saved checkpoint to load weights from')

parser.add_argument('--face', type=str, default='test.mp4',
	...
	for i, (img_batch, mel_batch, frames, coords) in enumerate(tqdm(gen, 
		...
		for p, f, c in zip(pred, frames, coords):
			...
			if len(faces) > 0:
				...
				cv2.fillPoly(mask, [np.array(face_contour_points)], 255)
				for i in range(3):  # B, G, R
					p[:, :, i] = f[y1:y2, x1:x2, i] * (1 - (mask / 255)) + p[:, :, i] * (mask / 255)
